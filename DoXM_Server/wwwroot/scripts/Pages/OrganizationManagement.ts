...
document.getElementById("organizationNameInput").addEventListener("input", (ev) => {
    var addon = (ev.currentTarget as HTMLInputElement).parentElement.querySelector(".glyphicon");
    addon.classList.remove("glyphicon-ok");
    addon.classList.add("glyphicon-edit");
});
document.getElementById("organizationNameInput").addEventListener("blur", (ev) => {
    var xhr = new XMLHttpRequest();
    xhr.onload = () => {
        if (xhr.status == 200) {
            var addon = (ev.target as HTMLInputElement).parentElement.querySelector(".glyphicon");
            addon.classList.remove("glyphicon-edit");
            addon.classList.add("glyphicon-ok");
        }
